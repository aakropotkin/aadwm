...
arrange( Monitor * m )
{
  if ( m ) { showhide( m->cl->stack ); }
  else
    {
      for ( m = mons; m; m = m->next ) { showhide( m->cl->stack ); }
    }
  if ( m )
    {
      arrangemon( m );
...
attachclients( Monitor * m )
{
  ...
  for ( tm = mons; tm; tm = tm->next )
    {
      if ( tm != m ) { utags |= tm->tagset[tm->seltags]; }
    }
...
{
  Client ** tc = NULL;
  for ( tc = &c->mon->cl->clients; ( ( *tc ) != NULL ) && ( ( *tc ) != c );
        tc = &( *tc )->next )
    ;
  *tc = c->next;
}
...
detachstack( Client * c )
{
  ...
  for ( tc = &c->mon->cl->stack; ( ( *tc ) != NULL ) && ( ( *tc ) != c );
        tc = &( *tc )->snext )
    ;
  *tc = c->snext;
...
focusmon( const Arg * arg )
{
  ...
  if ( ! mons->next ) return;
  if ( ( m = dirtomon( arg->ui ) ) == selmon ) return;
...
focusstack( const Arg * arg )
{
  ...
  if ( 0 < arg->i )
    {
      for ( c = selmon->sel->next; c && ( ! ISVISIBLE( c, selmon ) );
            c = c->next )
        ;
      if ( ! c )
        {
        ...
        if ( m != selmon && m->tagset[m->seltags] & newtags )
          {
            ...
            if ( newtags & selmon->tagset[selmon->seltags] ) return;
...
tagmon( const Arg * arg )
{
  ...
  sendmon( selmon->sel, dirtomon( arg->ui ) );
}
...
tile_vsplit( Monitor * m )
{
  ...
  for ( n = 0, c = nexttiled( m->cl->clients, m ); c != NULL;
  ...
  if ( m->nmaster == 0 ) mw = 0; /* Empty */
    ...
    mw = m->ww * m->mfact; /* Split vertically */
  else
    mw = m->ww; /* Full width */

  /* Resize and move clients. */
  for ( i = 0, my = 0, ty = 0, c = nexttiled( m->cl->clients, m ); c != NULL;
...
tile_hsplit( Monitor * m )
{
  ...
  if ( m->nmaster == 0 ) mh = 0; /* Empty */
    ...
    mh = m->wh * m->mfact; /* Split horizontally */
  else
    mh = m->wh; /* Full height */

  /* Resize and move clients. */
  for ( i = 0, mx = 0, tx = 0, c = nexttiled( m->cl->clients, m ); c != NULL;
        c = nexttiled( c->next, m ), i++ )
    {
      /* Handle members of the master tile vs reular tile. */
      if ( i < m->nmaster )
...
view( const Arg * arg )
{
  ...
  if ( arg->ui & TAGMASK ) { newtagset = arg->ui & TAGMASK; }
  for ( m = mons; m; m = m->next )
    {
      if ( ( m != selmon ) && ( newtagset & m->tagset[m->seltags] ) )
        {
           ...
           * are connected */
          if ( newtagset & selmon->tagset[selmon->seltags] ) { return; }
          m->sel = selmon->sel;
          m->seltags ^= 1;
...
zoom( const Arg * arg )
{
  ...
  if ( c == nexttiled( selmon->cl->clients, selmon ) )
    {
      if ( ( ! c ) || ( ! ( c = nexttiled( c->next, selmon ) ) ) ) { return; }
    }
  pop( c );
}
